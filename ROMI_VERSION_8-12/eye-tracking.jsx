...
const EyeTracking = () => {
  ...
  const fetchFixationLossCounter = async () => {
    try {
      const response = await fetch('http://10.100.137.250:5000/fixation_loss_counter');
  ...
  const startTracking = async () => {
    try {
      await fetch('http://10.100.137.250:5000/start_tracking', { method: 'POST' });
  ...
  const pauseTracking = async () => {
    try {
      await fetch('http://10.100.137.250:5000/pause_tracking', { method: 'POST' });
  ...
  return (
    <View style={styles.container}>
      ...
      <WebView
        source={{ uri: 'http://10.100.137.250:5000/video_feed' }}
