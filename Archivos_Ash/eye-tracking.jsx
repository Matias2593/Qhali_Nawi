...
const EyeTracking = () => {
  ...
  const fetchFixationLossCounter = async () => {
    try {
      const response = await fetch('http://10.100.68.197:5000/fixation_loss_counter');
      const data = await response.json();
      setFixationLossCounter(data.fixation_loss_counter);

      // Verificar si se ha alcanzado el límite de pérdidas de fijación
      if (data.fixation_loss_counter >= 5) {
  ...
  const startTracking = async () => {
    try {
      await fetch('http://10.100.68.197:5000/start_tracking', { method: 'POST' });
  ...
  const pauseTracking = async () => {
    try {
      await fetch('http://10.100.68.197:5000/pause_tracking', { method: 'POST' });
  ...
  return (
    <View style={styles.container}>
      ...
      <WebView
        source={{ uri: 'http://10.100.68.197:5000/video_feed' }}
