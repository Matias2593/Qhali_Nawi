...
log_file = "fixation_loss_log.txt"  # Nombre del archivo de registro


def log_fixation_loss():
    """Registra la pérdida de fijación con la hora exacta en un archivo de texto."""
    global fixation_loss_counter
    with open(log_file, "a") as file:
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        file.write(f"Loss {fixation_loss_counter}: Detected at {current_time}\n")
...
def generate_frames():
    global cap, fixation_loss_counter, is_tracking, reconnect_message_shown  # Declarar todas las variables globales
    ...
    while True:
        ...
        reconnect_message_shown = False  # Resetear el mensaje si se reconecta exitosamente
        ...
        if is_tracking and not pupil_detected:
            ...
            log_fixation_loss()  # Registrar la pérdida en el archivo
